...
from bucket_wrapper import config
from bucket_wrapper.views import (
    WorkspaceStatus,
...
def mock_k8s_base():
    with mock.patch(
        "bucket_wrapper.views.current_namespace",
...
def mock_remote_backend_harbor():
    with mock.patch(
        "bucket_wrapper.config.HARBOR_URL",
...
def mock_remote_backend_bucket():
    with mock.patch(
        "bucket_wrapper.config.BUCKET_ENDPOINT_URL",
...
def mock_read_namespace():
    ...
    with mock.patch(
        "bucket_wrapper.views.k8s_client.CoreV1Api.read_namespace",
...
def mock_dynamic_client_apply():
    with mock.patch("bucket_wrapper.views.DynamicClient") as mocker:
        yield mocker
        # "bucket_wrapper.views.DynamicClient.resources.get.server_side_apply",
...
def mock_read_secret():
    ...
    with mock.patch(
        "bucket_wrapper.views.k8s_client.CoreV1Api.read_namespaced_secret",
...
def mock_create_secret():
    with mock.patch(
        "bucket_wrapper.views.k8s_client.CoreV1Api.create_namespaced_secret"
    ) as mocker:
        yield mocker
...
def mock_create_namespace():
    with mock.patch(
        "bucket_wrapper.views.k8s_client.CoreV1Api.create_namespace"
    ) as mocker:
        yield mocker
...
def mock_delete_namespace():
    with mock.patch(
        "bucket_wrapper.views.k8s_client.CoreV1Api.delete_namespace"
    ) as mocker:
        yield mocker
...
def mock_read_config_map():
    with mock.patch(
        "bucket_wrapper.views.k8s_client.CoreV1Api.read_namespaced_config_map",
...
def mock_list_ingress():
    with mock.patch(
        "bucket_wrapper.views.k8s_client.NetworkingV1Api.list_namespaced_ingress",
...
def mock_patch_config_map():
    with mock.patch(
        "bucket_wrapper.views.k8s_client.CoreV1Api.patch_namespaced_config_map",
        return_value={},
    ) as mocker:
        yield mocker
...
def mock_wait_for_secret(mock_secret):
    with mock.patch(
        "bucket_wrapper.views.wait_for_namespace_secret",
...
def test_wait_for_secret_terminates_on_secret():
    ...
    with mock.patch(
        "bucket_wrapper.views.kubernetes.watch.Watch.stream",
